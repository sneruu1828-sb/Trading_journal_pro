...
// Store for all changes with incremental sync token
...
function generateSyncToken() {
  currentSyncToken++;
  return `${Date.now()}-${currentSyncToken}`;
}
...
// NEW SYNC ENDPOINTS FOR REAL CROSS-DEVICE SYNCING

// POST /sync/changes endpoint: receive changes from clients and store
app.post('/sync/changes', authenticate, handleIdempotency, (req, res) => {
  try {
    ...
    console.log(`Received ${changes.length} changes from user ${req.userId}`);

    // Assign a new token to each change and push to global log
    changes.forEach(change => {
      ...
      console.log(`Added change to log: ${change.type} ${change.action} ${change.data.id}`);
    });

    const response = {
      status: 'ok',
      newSyncToken: currentSyncToken,
...
// GET /sync/changes endpoint: send changes since client's last sync token
app.get('/sync/changes', authenticate, (req, res) => {
  try {
    const since = parseInt(req.query.since) || 0;
    const userId = req.userId;

    console.log(`Fetching changes since token ${since} for user ${userId}`);

    // Find all changes with token greater than 'since' for this user
    const changesSince = changesLog
      .filter(change => change.syncToken > since && change.userId === userId)
      .map(change => ({
        type: change.type,
        action: change.action,
        data: change.data,
        timestamp: change.timestamp,
        syncToken: change.syncToken
      }));

    console.log(`Found ${changesSince.length} changes since token ${since}`);
...
// ENHANCED FULL SYNC ENDPOINT - handles both upload and download with full data
app.post('/api/sync', authenticate, handleIdempotency, async (req, res) => {
  try {
    const { trades = [], strategies = [], lastSyncToken, deviceId } = req.body;
    const userId = req.userId;

    console.log(`Full sync for user ${userId}: ${trades.length} trades, ${strategies.length} strategies`);
    console.log(`Last sync token: ${lastSyncToken}, Device: ${deviceId}`);
    ...
    const sinceTime = lastSyncToken ? new Date(lastSyncToken.split('-')[0]).getTime() : 0;
    
    const serverTrades = Array.from(userDb.trades.values())
      .filter(trade => {
        const updateTime = new Date(trade.serverUpdatedAt || trade.updatedAt).getTime();
        return updateTime > sinceTime;
      });

    const serverStrategies = Array.from(userDb.strategies.values())
      .filter(strategy => {
        const updateTime = new Date(strategy.serverUpdatedAt || strategy.updatedAt).getTime();
        return updateTime > sinceTime;
      });
    ...
    // Log sync activity
    console.log(`Full sync completed for user ${userId}: applied ${changedTrades.length} trades, ${changedStrategies.length} strategies`);
    console.log(`Returning ${serverTrades.length} server trades, ${serverStrategies.length} server strategies`);
...
app.listen(PORT, () => {
  ...
  console.log('Real-time sync endpoints available:');
  console.log('- POST /sync/changes (simple sync)');
  console.log('- GET /sync/changes (pull changes)');
